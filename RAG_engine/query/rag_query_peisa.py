...
# ────────────────────────────────────────────────────────────────────────────
def get_peisa_product_by_model(model_name: str) -> dict | None:
    """Busca y retorna un producto de PEISA por su modelo (case-insensitive)."""
    global _CONN
    ...
    try:
        sql = """
            SELECT rowid, type, family, model, description,
                   dimentions, power_w, liters, max_pressure_bar
            FROM products WHERE LOWER(model) = ?
        """
        row = _CONN.execute(sql, (model_name.lower().strip(),)).fetchone()
        if row:
            return {
                "type": row[1],
                "family": row[2],
                ...
                "power_w": row[6],
                "liters": row[7],
                "max_pressure_bar": row[8]
            }
    except Exception as e:
        print(f"[PeisaQuery] Error buscando producto por modelo: {e}")
    return None
