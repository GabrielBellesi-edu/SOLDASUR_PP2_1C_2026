...
class OllamaLLM:
    ...
    def __init__(self, model: str = "llama3.2:3b"):
        ...
        # Configuración de System Prompt de fallback estático
...
- NO uses HTML (target, class, etc.) - solo texto natural y humanizado"""
        ...
        # Intentar cargar la configuración externa
        ...
        if config_path.exists():
            try:
                with open(config_path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    peisa_config = data.get("peisa", {})
                    self.system_prompt = peisa_config.get("system_prompt", self.system_prompt)
                    self.default_temperature = peisa_config.get("temperature", self.default_temperature)
                    self.default_max_tokens = peisa_config.get("max_tokens", self.default_max_tokens)
                    print("[PeisaRAG] Configuración de prompts cargada dinámicamente con éxito.")
            except Exception as e:
                print(f"[PeisaRAG] Error cargando prompts.json ({e}). Usando fallback.")
    ...
    def generate(self, 
                 ...
                 last_active_product: Optional[str] = None) -> str:
        """
        ...
        Args:
            ...
            last_active_product: Producto activo en el contexto
        """
        try:
    ...
    def _build_prompt(self, question: str, context: Optional[List[Dict]] = None, last_active_product: Optional[str] = None) -> str:
        ...
        if last_active_product:
            prompt_parts.append(f"\nContexto de conversación: El usuario acaba de recibir una recomendación sobre el producto '{last_active_product}' (o estaba hablando de él). Si la consulta del cliente es una pregunta de seguimiento (como colores, rendimiento, secado, precio, alternativas, etc.), responde asumiendo que se refiere a este producto y sugiere complementos o alternativas relevantes para el mismo.")
...
def answer(question: str, context: List[Dict] = None, last_active_product: Optional[str] = None) -> str:
    """Función de compatibilidad con versión anterior"""
    return llm.generate(question, context, last_active_product=last_active_product)
