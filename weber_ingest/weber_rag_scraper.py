        ...
        for i in range(retries):
            try:
                ...
                if self.page.is_closed():
                    self.context = await self.playwright.chromium.launch_persistent_context(
                        "./user_data",
                        headless=False,    # Para auditar visualmente que el scraper no fuera bloqueado
                        args=["--disable-blink-features=AutomationControlled"],
                    )
                    self.page = self.context.pages[0]
                ...
                if "momento" in title.lower() or "checking" in title.lower() or "cloudflare" in title.lower():
                    ...
                    if resolved:
                        ...
                        await asyncio.sleep(2)  # Esperar a que renderice la página del producto
                    else:
    ...
    async def scrape_producto(self, producto: dict) -> dict:
        ...
        try:
            ...
            datos = {
                ...
                "url": producto["url"],
                "descripcion": "",
                "datos_tecnicos": "", # Nuevo campo para guardar todo lo que encuentre
                "documentos": [],
