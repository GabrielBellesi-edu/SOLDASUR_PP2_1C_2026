...
OUTPUT_DIR = Path("scraping/data_raw/weber")
...
def crear_directorios():
    for sub in ["productos", "documentos", "chunks", "raw", "img"]:
...
class WeberScraper:
    ...
    async def scrape_producto(self, producto: dict) -> dict:
        ...
        try:
            ...
            # --- NUEVO: Extracción estructurada de taxonomías y atributos técnicos (v4.3.0) ---
            filtros = {}
            atributos_tecnicos = {}
            
            # Encontrar el cuerpo principal
            cuerpo_para_estructurar = soup.find("main") or soup.find("article") or soup.find("div", {"id": "main-content"})
            if cuerpo_para_estructurar:
                # Clonar para poder limpiar menús y elementos de navegación globales
                cuerpo_clon = BeautifulSoup(str(cuerpo_para_estructurar), "html.parser")
                
                # Decomponer navegación global
                for tag in cuerpo_clon.find_all(["header", "nav", "footer", "aside"]):
                    tag.decompose()
                for tag in cuerpo_clon.find_all(class_=lambda c: c and any(k in c for k in ["navbar", "menu", "header", "footer", "breadcrumb", "modal"])):
                    tag.decompose()
                
                # 1. Extraer taxonomías (links de filtros)
                for a in cuerpo_clon.find_all("a", href=True):
                    href = a["href"].strip()
                    if "/search-content/content_type/product/" in href:
                        ...
                        if len(parts) >= 5:
                            ...
                            mapping = {
                                "product_benefits": "beneficios",
                                "activities": "actividades",
                                "area": "area",
                                "application_characteristics": "aplicacion",
                                "colors": "colores",
                                "juntas": "juntas"
                            }
                            friendly_cat = mapping.get(cat, cat)
                            
                            if friendly_cat not in filtros:
                                filtros[friendly_cat] = []
                            
                            # Evitar duplicados
                            if val not in [item["slug"] for item in filtros[friendly_cat]]:
                                filtros[friendly_cat].append({
                                    "nombre": text,
                                    "slug": val
                                })
                
                # 2. Extraer atributos técnicos en formato h2 + p en la columna derecha
                col_derecha = cuerpo_clon.find(class_=lambda c: c and "col-sm-6" in c and "col-lg-7" in c)
                if not col_derecha:
                    col_derecha = cuerpo_clon
                    
                for h2 in col_derecha.find_all("h2"):
                    key = h2.get_text(strip=True)
                    if any(k in key.lower() for k in ["características", "beneficios", "restricciones", "documentación", "tutoriales"]):
                        continue
                    p = h2.find_next_sibling("p")
                    if p:
                        val = p.get_text(strip=True)
                        atributos_tecnicos[key] = val
            # ----------------------------------------------------------------------------------
            ...
            datos = {
                ...
                "datos_tecnicos": "", 
                "filtros": filtros,
                "atributos_tecnicos": atributos_tecnicos,
                "documentos": [],
            ...
            # 4. Captura de Imagen del Producto (Open Graph o Galería)
            img_meta = soup.find("meta", property="og:image") or soup.find("meta", {"name": "og:image"})
            img_url = img_meta["content"].strip() if img_meta and img_meta.get("content") else None
            
            if not img_url:
                img_gallery = soup.find("div", class_=lambda c: c and "product-image" in c)
                if img_gallery:
                    img_tag = img_gallery.find("img")
                    if img_tag and img_tag.get("src"):
                        img_url = img_tag["src"].strip()
            
            datos["imagen_url"] = urljoin(BASE_URL, img_url) if img_url else None
    ...
    # ──────────────────────────────────────────
    # 3b. DESCARGAR IMAGEN VIA NAVEGADOR INYECCIÓN JS
    # ──────────────────────────────────────────
    async def descargar_imagen(self, img_url: str, nombre_producto_slug: str) -> str | None:
        if not img_url:
            return None
        ...
        nombre_archivo = f"{nombre_producto_slug}{ext}"
        ruta_local = OUTPUT_DIR / "img" / nombre_archivo

        if ruta_local.exists() and ruta_local.stat().st_size > 1000:
            return str(ruta_local)

        try:
            log(f"  📥 Descargando imagen: {nombre_archivo}")
            b64_data = await self.page.evaluate(f"""
                async () => {{
            ...
            if len(img_bytes) > 1000:
                ruta_local.write_bytes(img_bytes)
                tamanio = len(img_bytes) // 1024
                log(f"  ✅ Imagen guardada: {nombre_archivo} ({tamanio} KB)")
                return str(ruta_local)
            
            return None

        except Exception as e:
            log(f"  ❌ Falló descarga de imagen {nombre_archivo}: {e}")
            return None
...
# PIPELINE PRINCIPAL
# ─────────────────────────────────────────────
async def main(solo_productos=False, max_productos=None, descargar_pdfs=True, descargar_imagenes=True):
    crear_directorios()
    ...
    try:
        ...
        for i, prod in enumerate(tqdm(lista, desc="Progreso")):
            ...
            if descargar_imagenes and detalle.get("imagen_url"):
                ruta_img = await scraper.descargar_imagen(detalle["imagen_url"], nombre_archivo)
                if ruta_img:
                    detalle["imagen_local"] = str(Path(ruta_img).relative_to(OUTPUT_DIR.parent.parent))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--solo-productos",  action="store_true", help="Sin descargar PDFs o imágenes")
    ...
    parser.add_argument("--sin-img",         action="store_true", help="No descargar imágenes")
    args = parser.parse_args()

    asyncio.run(main(
        solo_productos=args.solo_productos,
        max_productos=args.max_productos,
        descargar_pdfs=not (args.sin_pdfs or args.solo_productos),
        descargar_imagenes=not (args.sin_img or args.solo_productos),
    ))
