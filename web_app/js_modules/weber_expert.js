...
function calcularMaterialesWeber(soporte, m2, context) {
    ...
    if (config.tipo === 'adhesivo') {
        ...
        if (context.doble_encolado === 'si') {
            rendimiento *= 1.5;
            detalleExtra = '• <em>Incluye un incremento del 50% por técnica de doble encolado (piezas > 30 cm).</em><br>';
        }
...
function iniciarExpertoWeber() {
    ...
    const textoCompleto = `<strong>🧱 Asesor Experto Weber:</strong> Te voy a guiar para calcular los materiales necesarios para tu obra.<br><br>${nodoInicio.pregunta}`;
...
function _ejecutarCalculoWeber() {
    ...
    const textoResultado = `
        ...
        <span class="text-xs text-gray-500">${resultado.descripcion}</span><br><br>
        ...
        • <strong>Material total estimado (+10% desperdicio):</strong> ${resultado.kg_totales} kg/L.
    `;
